        ...
        <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${isScrolled ? 'bg-white/80 backdrop-blur-md py-4 shadow-sm' : 'bg-transparent py-6'
            ...
            <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                ...
                <div className="hidden md:flex items-center gap-8">
                    <Link to="/products" className="text-sm font-bold uppercase tracking-widest text-zinc-600 hover:text-zinc-900 transition-colors">
                        Cửa hàng
                    </Link>
                    ...
                    {user && user.role && user.role.toUpperCase() === 'ADMIN' && (
                        <Link to="/admin" className="text-sm font-bold uppercase tracking-widest text-red-650 hover:text-red-800 transition-colors">
                            Admin
                        </Link>
                    )}
                    {user && user.role && (user.role.toUpperCase() === 'MANAGER' || user.role.toUpperCase() === 'ADMIN') && (
                        <Link to="/manager" className="text-sm font-bold uppercase tracking-widest text-indigo-600 hover:text-indigo-850 transition-colors">
                            Quản lý
                        </Link>
                    )}
                </div>
                ...
                <div className="flex items-center gap-5">
                    ...
                    {user ? (
                        <div className="flex items-center gap-4 border-l border-zinc-200 pl-5">
                            <Link to="/profile" className="flex items-center gap-1.5 hover:text-emerald-600 transition-colors">
