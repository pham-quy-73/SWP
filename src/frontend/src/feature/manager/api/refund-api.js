...
export const refundApi = {
  ...
  inActivateVariant: async (variantId) => {
    await httpClient.patch(`/api/refund/variant/${variantId}/in-activate`);
  },

  // Bước 2: Lấy danh sách đơn hàng bị ảnh hưởng
  getAffectedOrders: async (variantId) => {
    const res = await httpClient.get(`/api/refund/affected-orders/${encodeURIComponent(variantId)}`);
    return res.data?.result || [];
  },

  // [Khách hủy] Lấy đơn đã hủy có thanh toán thành công
  getCancelledPaidOrders: async (params) => {
    const res = await httpClient.get(`/api/management/orders/cancelled/paid`, { params });
  ...
  createBatch: async (orderIds) => {
    const res = await httpClient.post(`/api/refund/create-batch`, { orderIds });
    return res.data?.result || [];
  },

  // Bước 4: Lấy danh sách refund sẵn sàng xử lý
  getReadyRefunds: async () => {
    const res = await httpClient.get(`/api/refund/ready`);
    return res.data?.result || [];
  },
