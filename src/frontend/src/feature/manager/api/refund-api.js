import { httpClient } from '../../../lib/httpClient';

export const refundApi = {
  // Bước 1: Vô hiệu hóa variant
  inActivateVariant: async (variantId) => {
    await httpClient.patch(`/refund/variant/${variantId}/in-activate`);
  },

  // Bước 2: Lấy danh sách đơn hàng bị ảnh hưởng
  getAffectedOrders: async (variantId) => {
    const res = await httpClient.get(`/refund/affected-orders/${encodeURIComponent(variantId)}`);
    return res.data?.result || [];
  },

  // [Khách hủy] Lấy đơn đã hủy có thanh toán thành công
  getCancelledPaidOrders: async (params) => {
    const res = await httpClient.get(`/management/orders/cancelled/paid`, { params });
    return res.data?.result || { items: [], page: 0, size: 10, totalElements: 0, totalPages: 0 };
  },

  // Bước 3: Tạo batch hoàn tiền
  createBatch: async (orderIds) => {
    const res = await httpClient.post(`/refund/create-batch`, { orderIds });
    return res.data?.result || [];
  },

  // Bước 4: Lấy danh sách refund sẵn sàng xử lý
  getReadyRefunds: async () => {
    const res = await httpClient.get(`/refund/ready`);
    return res.data?.result || [];
  },

  // Bước 5: Xác nhận hoàn tiền -> trả về url VNPay
  checkoutRefund: async (refundId) => {
    const res = await httpClient.post(`/refund/${refundId}/refund-checkout`);
    return res.data?.result || null;
  },

  // Từ chối yêu cầu hủy đơn hàng
  rejectCancellation: async (orderId, reason) => {
    const res = await httpClient.put(`/refund/reject-cancel/${orderId}`, { reason });
    return res.data?.result || null;
  },
};
