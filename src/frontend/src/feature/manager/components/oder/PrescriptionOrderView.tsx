...
export interface OrderItemPrescription {
  ...
  unitPrice: number;
  totalPrice: number;
  orderItemType: 'PRESCRIPTION' | 'IN_STOCK' | 'PRE_ORDER';
  prescription?: PrescriptionData | null;
}

export interface PrescriptionOrderProps {
  orderId: string;
  orderStatus: 'PENDING' | 'AWAITING_VERIFICATION' | 'PROCESSING' | 'PRODUCED' | 'SHIPPED' | 'COMPLETED' | 'ON_HOLD' | 'CANCELLED';
  customerName: string;
  ...
  onVerifyOrder?: (approved: boolean, note?: string) => void;
}
...
function EyeSpecsMatrix({ prescription }: { prescription: PrescriptionData }) {
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-xl w-full p-4 space-y-3 shadow-2xl relative">
            ...
            <div className="max-h-[70vh] overflow-auto flex justify-center bg-slate-100 rounded-xl p-2 border">
              <img src={prescription.imageUrl} alt="Prescription Scan" className="max-w-full h-auto object-contain rounded-lg" />
            </div>
