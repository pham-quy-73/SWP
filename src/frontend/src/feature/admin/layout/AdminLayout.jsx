    ...
    <div className="min-h-screen bg-slate-50 flex font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 text-slate-400 transition-all duration-300 flex flex-col shrink-0 ${collapsed ? 'w-16' : 'w-64'}`}>
        <div className="h-16 flex items-center justify-between px-4 border-b border-slate-800 shrink-0">
          {!collapsed && <span className="font-extrabold text-white text-lg tracking-tight px-2">ADMIN CENTRAL</span>}
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        <header className="h-16 bg-white border-b border-slate-205 flex items-center justify-between px-8 shrink-0 shadow-sm">
          <span className="font-bold text-slate-800 tracking-tight">Trang Quản Trị Hệ Thống (Admin)</span>
          <div className="flex items-center gap-3">
            <span className="text-xs font-bold text-slate-500 bg-red-100/80 px-3 py-1 rounded-full uppercase border border-red-200">
              Administrator
            </span>
          </div>
        </header>
