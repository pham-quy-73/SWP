...
| **U003** | ✅ `PUT /api/users/:id/role` đổi vai trò | `UserController.updateUserRole` | 1.0h | U001 | Validate role ∈ {CUSTOMER,MANAGER,ADMIN} (SALE và SHIPPER đã loại bỏ — out of scope); cập nhật `user.role`, trả 200; 400 khi role rỗng/không hợp lệ; 404 khi user không tồn tại. |
