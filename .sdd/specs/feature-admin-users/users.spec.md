...
- **N-3:** WHERE a locked user (`deleted_at !== null`) attempts to log in, THE `authenticate` middleware SHALL reject the request with HTTP 403 `ACCOUNT_LOCKED` — preventing reuse of previously issued tokens.

- **N-4:** WHERE target user ID does not exist in database, THE system SHALL return HTTP 404 `USER_NOT_FOUND` for ALL operations (view, update role, update status, delete, reset password).

- **N-5:** WHERE Admin attempts to reset password of another ADMIN account, THE system SHALL return HTTP 403 `FORBIDDEN` and SHALL NOT apply the change.

- **N-6:** WHERE `password` field is requested in any response, THE system SHALL exclude it via `.select('-password')` (E-1, getUserById, getMe).

---

## 4. Non-functional Requirements (Yêu cầu phi chức năng)

- **NFR-1 (Security — Authentication):** ALL User Management endpoints (except `/me`, `/me/change-password`) SHALL require both `authenticate` AND `requireRole(['ADMIN'])` middleware — enforced via `router.use()` group in `user.routes.js`.

- **NFR-2 (Security — Password Hashing):** ALL passwords SHALL be hashed using bcrypt with salt rounds = 12, triggered automatically by Mongoose `pre('save')` hook. Raw passwords SHALL NEVER be stored.

- **NFR-3 (Security — CSRF Prevention):** Role update SHALL accept `role` from `req.body` ONLY (not from query params) to prevent CSRF attacks via URL manipulation.

- **NFR-4 (Performance — Pagination):** `getAllUsers` SHALL use `skip/limit` pagination with `Promise.all([find, countDocuments])` for parallel execution. Default: 20 records/page.

- **NFR-5 (Data Privacy):** The `password` field SHALL be excluded from ALL API responses using Mongoose `.select('-password')`.
...
## 7. Frontend Architecture (Kiến trúc Giao diện)

### 7.1 Trang quản lý

- **Page:** `src/frontend/src/feature/admin/page/UserManagePage.jsx`
- **Route:** Chỉ truy cập được bởi ADMIN (bảo vệ bởi `PrivateRoute` + kiểm tra role).
...
- Mỗi mutation hook sử dụng `toast.loading()` → `toast.success()` / `toast.error()` để phản hồi trạng thái.
- Token đăng nhập lấy từ `localStorage.getItem('accessToken')`.
- API URL cấu hình qua biến môi trường `VITE_API_URL`.

---

## 8. Test Coverage (Phạm vi Kiểm thử)
