...
> **CHANGELOG**
> - **v1.1.0 (2026-06-20):** Mở rộng Risks từ 1 → 6. Bổ sung §7 "Questions for Human". Ghi chú lệch đường dẫn file với TASKS.md. **Không thay đổi kiến trúc/quyết định kỹ thuật.**
...
> ✅ **Đường dẫn đã đồng bộ với repo thực tế (2026-06-23):** 🆕 Trước đây PLAN ghi một số path không khớp code (`services/CloudinaryService.js`, `product.routes.js`, `features/products/...`, casing PascalCase). Đã sửa lại đúng cấu trúc thư mục thực tế bên dưới.

**Backend (Node.js/Express):**
- `src/backend/models/Product.js`: Mongoose Schema. Đánh **2 single index** trên `name` và `brand`. Tích hợp trường `deleted_at: Date` (mặc định `null`) phục vụ Soft-delete.
- `src/backend/config/cloudinaryConfig.js`: Khởi tạo và tương tác với SDK Cloudinary.
...
- `src/backend/controllers/productController.js` & `src/backend/routes/productRoutes.js`: Định tuyến endpoint phẳng `/api/products`, tích hợp Middleware `verifyToken` và `checkRole('ADMIN')` (đã bật lại — xem DEV-01).

**Frontend (React/Vite):**
- `src/frontend/src/feature/products/hooks/useProducts.js`: Đảm nhiệm fetch API, quản lý URL Query Params.
- `src/frontend/src/feature/products/components/ProductGrid.jsx` & `src/frontend/src/feature/products/components/ProductCard.jsx`: Giao diện khách hàng.
- `src/frontend/src/feature/admin/ProductFormModal.jsx`: Form tạo/sửa sản phẩm kèm tính năng Preview ảnh.
- `src/frontend/src/pages/ProductDetailPage.jsx`: Trang chi tiết sản phẩm (T8).
...
*(Các điểm spec còn mơ hồ cần con người chốt trước khi code.)*
...
3. ~~**Pagination mặc định:** `limit` default = bao nhiêu, có giới hạn `limit` tối đa?~~ **RESOLVED (2026-06-23):** `limit` default = **12**, **tối đa 100** (chặn ở `listQuerySchema` + service `MAX_LIMIT`); `page` default = 1. Query sai kiểu → 400 (ERR-07).
