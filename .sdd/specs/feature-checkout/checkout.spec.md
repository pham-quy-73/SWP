...
- **Hủy đơn (Customer):** **[SYNC]**
  ...
  - **THE SYSTEM SHALL** chỉ cho phép khi status ∈ `{PENDING, AWAITING_VERIFICATION, CONFIRMED}`; kiểm tra quyền sở hữu (CUSTOMER chỉ hủy đơn của chính mình, MANAGER/ADMIN không bị giới hạn ownership); hoàn kho variant (`$inc`); chuyển `CANCELLED`; ghi `status_history` với note "Khách hàng hủy đơn. Lý do: …" (hoặc "Khách hàng chủ động hủy đơn hàng" nếu không có lý do); trả HTTP 200.
- **Hủy đơn (Manager/Admin qua cập nhật trạng thái):** **[SYNC]**
  - **WHEN** MANAGER/ADMIN gửi `PUT /api/orders/:id/status` với `{ status: "CANCELLED" }`.
  - **THE SYSTEM SHALL** áp dụng state machine: chỉ cho phép từ `{PENDING, AWAITING_VERIFICATION, CONFIRMED}` → `CANCELLED`; ADMIN được bypass state machine (override tự do kèm audit log); hoàn kho variant (`$inc`) giống luồng Customer cancel; ghi `status_history` với note từ `req.body.note` (ADMIN override tự sinh note mặc định); trả HTTP 200.
  - **Phân biệt với Customer cancel:** endpoint khác (`/status` vs `/cancel`), không kiểm tra ownership, note history khác nhau. Hành vi tồn kho và chuyển trạng thái **đồng nhất** giữa hai luồng.
...
- **Hành vi:** validate range trước, sau đó normalize:
  1. **Validate (ném 400 nếu sai):** SPH ∈ [-20, +20]; CYL ∈ [-6, +6]; AXIS ∈ [1, 180] (số nguyên); ADD ∈ [0.75, 4.0]; PD ∈ [20, 40]; CYL ≠ 0 bắt buộc có AXIS. Giá trị không phải số hợp lệ cũng ném lỗi.
  2. **Normalize (sau khi pass validate):** `undefined/null/empty` → `0`; note cắt 500 ký tự; ảnh toa (`imageUrl`) giữ nguyên từ bản gốc — endpoint này chỉ sửa thông số số.
  3. Ghi audit vào `status_history` của đơn (trạng thái không đổi).
- **Lỗi:** `INVALID_STATUS` (400) nếu đơn không ở `AWAITING_VERIFICATION`; `NO_LENS` (400) nếu item không gắn tròng; `ITEM_NOT_FOUND` (404); `VALIDATION_ERROR` (400) nếu thông số ngoài miền giá trị.
