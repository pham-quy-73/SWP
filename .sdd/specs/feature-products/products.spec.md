...
> **2026-07-22:** Tròng kính tách khỏi Product thành model riêng `Lens` (`models/Lens.js`,
> API `/api/lenses`, seed `seed_lenses.js`). Products API không còn category `LENS`
> (enum chỉ còn `FRAME | SUNGLASSES`), bỏ toàn bộ logic ẩn/lọc LENS. Các mục thay đổi
> đánh dấu **[LENS-SPLIT]**.
...
- **[LENS-SPLIT]** Tròng kính là sản phẩm khác bản chất gọng (không có variant/size/màu, chỉ có chất liệu + giá) — trước đây nhét chung vào Product với category `LENS` gây nhiễu cả API lẫn UI. Nay tách hẳn thành model `Lens` riêng với API `/api/lenses` (CRUD, soft-delete qua `status`); spec này chỉ còn quản lý gọng/kính mát.

**Mục tiêu:**

- Một API danh sách duy nhất `GET /api/products` phục vụ cả Khách (chỉ hàng `ACTIVE`) lẫn Manager (xem tất cả qua `status=ALL`).
...
**[ĐÃ VÁ — OQ-1]** Route đọc gắn `optionalAuthenticate`: có token MANAGER/ADMIN hợp lệ thì `req.user` xác định quyền staff (thấy `INACTIVE`); khách vãng lai hoặc token hỏng luôn bị khóa ở `ACTIVE`. Cờ `isManager` đã bị gỡ khỏi cả backend lẫn FE.
...
- **E-3:** **[LENS-SPLIT]** WHEN client truyền `category`, THE hệ thống SHALL lọc đúng giá trị đó; không truyền → không lọc category. KHÔNG còn logic ẩn/hiện `LENS` — tròng kính phục vụ qua `GET /api/lenses` (`ProductForm` fetch danh sách tròng từ đó, không còn gọi `?category=LENS`).
...
| `category`                                                                  | String enum            | `FRAME` \| `SUNGLASSES`, **required**, default `FRAME`           | **[LENS-SPLIT]** không còn `LENS` — tròng kính là model `Lens` riêng |
| `frameType`, `gender`, `shape`, `frameMaterial`, `hingeType`, `nosePadType` | String                 | default `''`                                                     | Thả lỏng, nhận chuỗi rỗng              |
...
- **EC-7:** **[ĐÃ VÁ — OQ-1]** Khách gọi tay `?status=ALL`/`?isManager=true` vẫn chỉ nhận hàng `ACTIVE`; quyền staff suy từ JWT, cờ client bị bỏ qua.
...
- **AC-1 (Danh sách mặc định):** _Given_ DB có sản phẩm `ACTIVE`, `INACTIVE` và bản ghi trong collection `lenses`, _When_ `GET /api/products` không query, _Then_ 200 và `items` chỉ chứa product `ACTIVE` (lens ở collection riêng, không bao giờ lẫn vào); envelope `{ code: 0, result: { items, page, size, totalElements, totalPages } }` với `page = 0`. **[LENS-SPLIT]**
- **AC-2 (Lọc & tìm kiếm):** _Given_ sản phẩm brand "Gucci" giá 2tr, _When_ `GET /api/products?search=guc&minPrice=1000000&maxPrice=3000000`, _Then_ sản phẩm xuất hiện trong `items` (search khớp cả `name` lẫn `brand`, không phân biệt hoa thường).
- **AC-3 (Phân trang):** _Given_ 15 sản phẩm ACTIVE, _When_ `?page=2&limit=10`, _Then_ `items.length = 5`, `page = 1`, `totalElements = 15`, `totalPages = 2`.
- **AC-4 (Tròng kính qua API riêng):** **[LENS-SPLIT]** _When_ `GET /api/lenses`, _Then_ 200 `{ success, count, data }` chứa tròng kính `ACTIVE`; _When_ `GET /api/products?category=LENS`, _Then_ 200 với `items` **rỗng** (Products API không biết đến LENS).
- **AC-5 (Chi tiết):** _When_ `GET /api/products/:id` hợp lệ, _Then_ 200 trả **bare object** Product; ID không tồn tại → 404 `NOT_FOUND`; ID sai định dạng → 400 `INVALID_ID`; product `INACTIVE` không kèm token staff → 404.
- **AC-6 (Tạo sản phẩm):** _Given_ token MANAGER, _When_ `POST /api/products` multipart (`product` JSON + 2 `files`), _Then_ 201 `{ code: 0, result }`, `result.imageUrl` có 2 entry `/uploads/...`, file tồn tại trên đĩa và truy cập được qua static route.
- **AC-7 (Tạo thiếu trường):** _When_ `POST` thiếu `brand`, _Then_ 400 và KHÔNG có bản ghi mới.
- **AC-8 (Phân quyền):** _When_ `POST /api/products` không token, _Then_ 401 `error_code: UNAUTHORIZED`; token CUSTOMER → 403 `error_code: FORBIDDEN`.
- **AC-9 (Cập nhật):** _Given_ product có ảnh cũ, _When_ `PUT` gửi lại `imageUrl` cũ + 1 file mới, _Then_ 200 và `imageUrl` = ảnh cũ + ảnh mới; _When_ `PUT` không gửi `imageUrl`, _Then_ ảnh cũ bị thay bằng chỉ file mới (hành vi đã biết — EC-2).
- **AC-10 (Xóa):** _When_ `DELETE /api/products/:id` bởi ADMIN, _Then_ 200, product biến mất khỏi `GET` danh sách VÀ toàn bộ variants của nó bị xóa theo (cascade); file ảnh `/uploads/...` liên quan được dọn best-effort.
- **AC-11 (CRUD variant):** _Given_ product tồn tại, _When_ `POST .../variants` với `variant` JSON hợp lệ, _Then_ 201 `{ success: true, result }` có `productId` đúng; `PUT` đổi `quantity` → 200 phản ánh ngay trong `GET .../variants`; `DELETE` → 200 và variant biến mất.
- **AC-12 (Variant của product ma):** _When_ `POST /api/products/<id-không-tồn-tại>/variants`, _Then_ 404 `{ success: false }`.
- **AC-13 (Hết hàng FE):** _Given_ mọi variant `quantity = 0`, _When_ khách mở trang chi tiết, _Then_ nút add-to-cart `disabled` + label "Sản phẩm tạm hết hàng", từng variant hiện badge "Hết hàng".
- **AC-14 (Deep-link filter):** _When_ mở `/products?category=SUNGLASSES&gender=FEMALE`, _Then_ bộ lọc khởi tạo đúng và request đầu tiên đã kèm 2 param đó.
- **AC-15 (Sort server-side):** _Given_ nhiều sản phẩm rải trên nhiều trang, _When_ `?sortBy=price-asc`, _Then_ `items` tăng dần theo `price` và đúng thứ tự **liên trang** (trang 2 tiếp nối trang 1); FE đổi sort → reset về trang 1.
- **AC-16 (Khóa quyền đọc):** _Given_ không có token, _When_ `GET /api/products?status=ALL`, _Then_ kết quả chỉ chứa `status: 'ACTIVE'`; _Given_ token MANAGER, _Then_ thấy cả `INACTIVE`.
- **AC-17 (Upload guard):** _When_ upload file 15MB hoặc file `.exe`, _Then_ 400 `FILE_TOO_LARGE`/`INVALID_FILE_TYPE` và không có file nào được ghi vào bản ghi.

### 7.1 Testing Requirements

- **Unit (BE):** query builder của `getProducts` (status default/ALL theo quyền staff, regex search, price range); validate 400 create; behavior wipe-imageUrl khi update (chốt hành vi hiện tại).
...
- **KHÔNG** quản lý tròng kính trong spec này — tròng là model `Lens` riêng với API `/api/lenses` (CRUD MANAGER, soft-delete qua `status: INACTIVE`). **[LENS-SPLIT]**
...
**[LENS-SPLIT]** Tròng kính có bộ endpoint riêng (Base: `/api/lenses`, envelope `{ success, count?, data }`): `GET /` (`?search=&status=ACTIVE|INACTIVE|ALL`), `GET /:id`, `POST /`, `PUT /:id`, `DELETE /:id` (soft-delete → `INACTIVE`). FE Manager quản lý qua tab "Tròng kính" (`useManagerLenses.js`); `ProductForm` fetch danh sách tròng từ đây.
