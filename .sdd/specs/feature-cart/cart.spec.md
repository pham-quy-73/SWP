...
- **E-10:** **[REALITY-2026-07-21]** WHEN khách chọn một tròng (`selectedLensId !== 'none'`) ở trang chi tiết, THE `ProductForm` SHALL render `PrescriptionWidget` (`PrescriptionModal.jsx`) để khách nhập đơn kính qua **ảnh** (upload → base64 `data:` URL vào `prescription.imageUrl`) hoặc **thủ công** (OD/OS: `sphere`/`cylinder`/`axis`/`add`/`pd`) + `notes`, lưu vào `usePrescriptionStore`. THE inputs SHALL be strictly validated in real-time and formatted on blur: SPH in `[-20.00..20.00]` (step 0.25), CYL in `[-6.00..6.00]` (step 0.25), AXIS in `[1..180]` (integer, mandatory if CYL !== 0), ADD in `[0.75..4.00]` (step 0.25), PD in `[20.0..40.0]` (step 0.5). WHEN `handleAddToCart`, THE system SHALL validate the prescription; IF invalid, THE system SHALL reject adding to cart, toast detailed validation errors, and keep the user on the page; IF valid and has data (`hasPrescriptionData`), THE payload `prescription` SHALL = `{ ...prescription }`, otherwise `prescription = null`. After successful add, `resetPrescription()` runs and `selectedLensId` resets to `'none'`.
